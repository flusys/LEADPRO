...
export class EncryptionService {
    private rsaPublicKey: WritableSignal<CryptoKey | null> = signal(null);
    public rsaPrivateKey: WritableSignal<CryptoKey | null> = signal(null);
    private http = inject(HttpClient);


    async loadPublicKey() {
        await this.importPemKey(`-----BEGIN PUBLIC KEY-----

-----END PUBLIC KEY-----`, false);
    }


    loadPublicKeyPEM() {
        return this.http.get('./public_key.pem', { responseType: 'text' });
    }

    // Generate RSA key pair for one-time use (run manually)
    async generateRSAKeysForOneTime() {
        ...
        this.rsaPublicKey.set(keyPair.publicKey);
        this.rsaPrivateKey.set(keyPair.privateKey);

        this.downloadPEMKey(await this.exportKeyToPEM(this.rsaPublicKey() as CryptoKey, false), 'public_key.pem');
        this.downloadPEMKey(await this.exportKeyToPEM(this.rsaPrivateKey() as CryptoKey, true), 'private_key.pem');
    }
    ...
    async decryptData(encryptedData: ArrayBuffer, iv: Uint8Array<ArrayBuffer>, aesKey: CryptoKey): Promise<string> {
    ...
    async encryptAESKey(aesKey: CryptoKey): Promise<ArrayBuffer> {
        if (!this.rsaPublicKey()) {
            throw new Error('RSA public key not initialized. Call importPemKey() first.');
        }
